...
import * as S from './Comment.style.ts';
...
export default function Comment({ comment, postId, onSuccess }: commentProps) {
  ...
  const { comments: replies, getCommentsInfo } = useComments(
    postId!,
    comment?.commentId ?? 0,
  );
    ...
    <S.Container>
      <S.ForRow>
          ...
          <S.ForColumn>
            ...
            {comment && comment.replyCount > 0 && (
              <S.ReplyButton onClick={() => setShowReplies(!showReplies)}>
                ...
                <S.ArrowIcon $isOpen={showReplies}>
                  <IoIosArrowBack />
                </S.ArrowIcon>
              </S.ReplyButton>
            )}
        ...
        {comment && kebabItems.length > 0 && (
          <S.KebabWrapper>
            <KebabMenu items={kebabItems} trigger={<S.KebabIcon />} />
          </S.KebabWrapper>
        )}
      </S.ForRow>
